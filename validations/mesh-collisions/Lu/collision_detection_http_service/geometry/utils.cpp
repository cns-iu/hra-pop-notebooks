...
void load_intersection_spaces(const std::string &intersection_space_dir, std::unordered_map<std::string, Mymesh>& intersection_space_dic) 
{
    for (fs::directory_entry& mesh_path : fs::directory_iterator(intersection_space_dir)) 
    {
        std::string mesh_file_name = mesh_path.path().filename().string();
        std::cout << "loading... " << mesh_file_name << std::endl;
        intersection_space_dic[mesh_file_name] = Mymesh(mesh_path.path().string());
        intersection_space_dic[mesh_file_name].create_aabb_tree();
    }
}
